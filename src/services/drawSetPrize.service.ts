import type { CreateDrawSetPrizeInput } from "@/schemas";
...
export class DrawSetPrizeService extends BaseService {
	...
	async autoAssignNumbers(drawSetId: string): Promise<void> {
		...
		const numbers = Array.from({ length: prizes.length }, (_, i) => i + 1);
	...
	async updateRarity(
		...
		if (
			existing.DrawSet.startTime &&
			existing.DrawSet.startTime <= new Date()
		) {
